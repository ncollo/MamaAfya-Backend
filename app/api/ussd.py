...
async def ussd_callback(
    ...
    # LEVEL 1: Symptom Reporting or Emergency
        ...
        if user_path[0] == "1":
            ...
            await notify_chw_dashboard(phoneNumber, "High Risk", ["emergency_button"])
    ...
    # LEVEL 2: Processing the chosen symptom
        ...
        if selected_option in options:
            ...
            if symptom_key == "other_escalate":
                ...
                await dispatch_emergency_sms(chw_phone, alert_msg)
                await notify_chw_dashboard(phoneNumber, "Medium Risk", ["unlisted_symptom"])
                ...
                report = SymptomReport(
                    ...
                    gestational_week=context.get("gestational_week", 0),
                    phase=phase
                )
                risk = calculate_risk(report)
                
                if risk.value == "High Risk":
                    alert_msg = f"URGENT: {context.get('first_name')} ({phoneNumber}) reported HIGH RISK: {options[selected_option]['text']}."
                    await dispatch_emergency_sms(chw_phone, alert_msg)
                    await notify_chw_dashboard(phoneNumber, risk.value, [symptom_key])
